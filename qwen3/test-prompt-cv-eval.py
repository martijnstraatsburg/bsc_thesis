...
# Description: This script prompts either of the Qwen3 models (4B or 8B) with a set of configurations and evaluates the results using cross-validation.
# Results are saved in desired output directory with basic comparing plots, JSON files for metrics and folds, and CSV aswell.
...
class CrossValidationEvaluator:
    ...
    def create_summary_report(self, all_results: Dict[str, Any]):
        ...
        for config_name, results in all_results.items():
            ...
            row = {
                ...
                "Weighted Precision": f"{metrics['weighted_precision']['mean']:.4f} ± {metrics['weighted_precision']['std']:.4f}",  # Added
                "Weighted Recall": f"{metrics['weighted_recall']['mean']:.4f} ± {metrics['weighted_recall']['std']:.4f}",  # Added
                "Weighted F1": f"{metrics['weighted_f1']['mean']:.4f} ± {metrics['weighted_f1']['std']:.4f}",  # Added
        ...
        for rating in ["suspense", "curiosity", "surprise"]:
            ...
            for config_name, results in all_results.items():
                ...
                row = {
                    ...
                    "Level Accuracy (Low/Med/High)": f"{metrics['level_accuracy']['mean']:.4f} ± {metrics['level_accuracy']['std']:.4f}",  # Added
    ...
    def create_summary_plots(self, all_results: Dict[str, Any]):
        ...
        for i, metric in enumerate(weighted_metrics):
            ...
            stds = [all_results[config]["average_metrics"]
                    ["story_classification"][metric]["std"] for config in configs]
            offset = width * (i - len(weighted_metrics)/2)
            rects = ax.bar(x + offset, means, width,
                           label=metric.replace("_", " ").title(), yerr=stds)
        ax.set_ylabel("Score")
        ax.set_title("Weighted Story Classification Metrics by Configuration")
        ax.set_xticks(x)
        ax.set_xticklabels([c.split("_shot")[0]
                           for c in configs], rotation=45, ha="right")
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir,
                    "weighted_story_classification_metrics.png"))
        ...
        for rating in ["suspense", "curiosity", "surprise"]:
            fig, ax = plt.subplots(figsize=(12, 6))
            accuracy_types = ["perfect_accuracy", "off_by_one_accuracy", "level_accuracy"]
            labels = ["Perfect Match", "Off-by-One", "Level Accuracy (Low/Med/High)"]
            x = np.arange(len(configs))
            width = 0.25
