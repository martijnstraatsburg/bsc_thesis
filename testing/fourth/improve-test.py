...
def run_ensemble_classification(client, input_file: str, model_name: str = MODEL, output_dir: str = "results"):
    """Run ensemble classification combining multiple shot techniques with voting."""
    os.makedirs(output_dir, exist_ok=True)

    with open(input_file, 'r', encoding='utf-8') as f:
        gold_standard = json.load(f)

    # Run each shot technique
    ...
    for shot in shot_techniques:
        ...
        # Create two analyzers with different configurations
        analyzer1 = PostAnalyser(client=client, model_name=model_name, shot=shot, temperature=0.1, chain_of_thought=True)
        analyzer2 = PostAnalyser(client=client, model_name=model_name, shot=shot, temperature=0.2, chain_of_thought=False)
        
        shot_results[shot] = []
        
        # Process each entry
        for entry in tqdm(gold_standard, desc=f"{shot}-shot"):
            text = entry["body"]
            
            # Get two analyses for the same text
            analysis1 = analyzer1.analyse_text(text)
            analysis2 = analyzer2.analyse_text(text)
            
            # Store both results
            shot_results[shot].append({
                "id": entry["id"],
                "body": text,
                "analysis1": analysis1,
                "analysis2": analysis2
    ...
    for i, entry in enumerate(gold_standard):
        ...
        classifications = []
        for shot in shot_techniques:
            classifications.append(shot_results[shot][i]["analysis1"]["story_class"])
            classifications.append(shot_results[shot][i]["analysis2"]["story_class"])
        ...
        avg_suspense = sum(shot_results[shot][i]["analysis1"].get("suspense", 1) + 
                         shot_results[shot][i]["analysis2"].get("suspense", 1) 
                         for shot in shot_techniques) / (len(shot_techniques) * 2)
        
        avg_curiosity = sum(shot_results[shot][i]["analysis1"].get("curiosity", 1) + 
                          shot_results[shot][i]["analysis2"].get("curiosity", 1) 
                          for shot in shot_techniques) / (len(shot_techniques) * 2)
        
        avg_surprise = sum(shot_results[shot][i]["analysis1"].get("surprise", 1) + 
                         shot_results[shot][i]["analysis2"].get("surprise", 1) 
                         for shot in shot_techniques) / (len(shot_techniques) * 2)
...
def process_gold_standard(client, input_file: str, model_name: str = MODEL, output_dir: str = "results"):
    ...
    run_ensemble_classification(client, input_file, model_name, output_dir)
...
if __name__ == "__main__":
    ...
    # Create a test configuration based on the best one
    if "ensemble" in best_config:
        analyser = PostAnalyser(client=client, model_name=MODEL, shot="multi", temperature=0.1, chain_of_thought=True)
    else:
        # Parse the best config name
        config_parts = best_config.split("_")
        shot = config_parts[0]
        temp = float(config_parts[2].replace("temp", ""))
        cot = config_parts[3] == "cotYes"
        
        analyser = PostAnalyser(client=client, model_name=MODEL, shot=shot, temperature=temp, chain_of_thought=cot)
